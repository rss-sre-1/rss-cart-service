pipeline {
    ...
    stages{
        stage('Load Test') {
            steps {
                sh "ls"
                sh "curl -F file=@cart-swagger.json -F 'LoadTestConfig={\"testPlanName\": \"CartService\", \"loops\": 1, \"threads\": 244, \"rampUp\": 1, \"followRedirects\" : false}' ${CONTEXT_PATH}${DOCUTEST} -o ${DOCUTEST_RESPONSE}"
